...
int pass_one(FILE* input, Block* blk, SymbolTable* table) {
  ...
  while (fgets(buf, BUF_SIZE, input)) {
    ...
    int label_result = add_if_label(len_blk, token, offset, table);
    if (label_result != 0) {
    ...
    while ((token = strtok(NULL, IGNORE_CHARS)) != NULL)
    {
      if (num_args >= MAX_ARGS) {
        raise_extra_argument_error(len_blk, token);
        error = -1;
        flag = 1;
        break;
      }
    ...
    if (num_args > MAX_ARGS || flag == 1) {
      continue;
    }
    blk->line_number = len_blk;

    int write_result = write_pass_one(blk, name, args, num_args);
    if (write_result == 0) {
      //printf("write_pass_one error in line %d\n", len_blk);
      //printf("name: %s\n", name);
      //printf("num_args: %d\n", num_args);
      raise_instruction_error(len_blk, name, args, num_args);
      error = -1;
    } else {
      offset += write_result * 4;
    }
  }
...
int pass_two(Block* blk, SymbolTable* table, FILE* output) {
  ...
  for (uint32_t i = 0; i < blk->len; ++i) {
    ...
    char* name = inst->name;
    char** args = inst->args;
    size_t num_args = inst->arg_num;
    
    // Calculate the byte offset for this instruction (4 bytes per instruction)
    uint32_t addr = i * 4;

    // Translate the instruction
    int translation_result = translate_inst(output, name, args, num_args, addr, table);
    //printf("translation_result: %d\n", translation_result);
    
    if (translation_result != 0) {
      // Translation error occurred
    //  printf("translation error in line %d\n", inst->line_number);
      //printf("name: %s\n", name);
  ...
  return error; // Return -1 if any errors were encountered, 0 otherwise
}
